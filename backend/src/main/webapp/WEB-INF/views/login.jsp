    ...
    <div class="auth-wrapper d-flex align-items-center justify-content-center py-5">
        <div class="bt-page-card" style="width: 100%; max-width: 420px;">
            <div class="card-body p-4">
                <h2 class="card-title text-center fw-bold mb-4">Entrar no Sistema</h2>

                <c:if test="${not empty mensagem}">
                    <div class="alert alert-success text-center"><c:out value="${mensagem}" /></div>
                </c:if>
                <c:if test="${not empty erro}">
                    <div class="alert alert-danger text-center"><c:out value="${erro}" /></div>
                </c:if>
                ...
                <form id="formLogin" action="${pageContext.request.contextPath}/login" method="post">
                    ...
                    <button type="submit" class="bt-btn bt-btn-accent w-100 fw-bold mt-2">Entrar</button>
                </form>

                <p class="text-center text-muted mt-3 mb-0">
                    Não tem conta? <a href="${pageContext.request.contextPath}/cadastro" class="fw-bold">Criar conta</a>
                </p>
            </div>
        </div>
    </div>
